...
class PerformanceMonitor:
    ...
    async def _loop(self) -> None:
        while self._active:
            try:
                ...
                logger.error("Monitor collection error: %s", ex)
    ...
    async def _collect(self) -> None:
        ...
        for agent_key, stats in agent_stats.items():
            ...
            for metric, value in [("agent_success_rate", sr), ("agent_avg_ms", ms)]:
                ...
                if anomaly:
                    logger.warning("Anomaly [%s] %s=%s z=%s", agent_key, metric, value, anomaly["z_score"])
        ...
        for tool_name, stats in tool_stats.items():
            sr = stats["success_rate"]
            ms = stats["avg_latency_ms"]
            cf = stats["consecutive_fails"]
            ...
            if cf >= 3:
                self._add_suggestion(
                    Suggestion(
                        ...
                        detail=f"成功率 {sr:.1%}，平均延迟 {ms:.0f}ms，熔断状态: {stats['circuit_state']}",
                        action="检查工具依赖服务、错误日志和超时配置。",
                        priority=9,
                    )
                )
    ...
    def _check_threshold(self, metric: str, value: float, label: str) -> None:
        ...
        triggered = (operator == "less_than" and value < threshold) or (operator == "greater_than" and value > threshold)
        if triggered:
            alert = Alert(
                severity=severity,
            ...
            if stats["success_rate"] < 0.85 and stats["total"] > 10:
                self._add_suggestion(
                    Suggestion(
                        ...
                        detail=f"成功率 {stats['success_rate']:.1%}，路由评分 {stats['routing_score']:.3f}",
                        action="检查 prompt、知识库和意图路由配置。",
                        priority=8,
                    )
                )

    def _add_suggestion(self, suggestion: Suggestion) -> None:
        if not any(item.title == suggestion.title for item in self._suggestions):
    ...
    async def _send_webhook(self, alert: Alert) -> None:
        ...
        try:
            ...
            logger.error("Webhook send failed: %s", ex)
